...
class DroneControllerTest {
	...
	void loadDroneWithMedicationsTestGood() throws Exception {
		long expectedId = 1L;
		when(droneService.loadDroneWithMedications(any(), any())).thenReturn(expectedId);
		String medicationListJSON = mapper.writeValueAsString(medicationList);
		String response = putRequest(HOST + DRONES_URL + "//" + DRONE_ID + LOAD_DRONE_WITH_MEDICATION,
				medicationListJSON, 200);
		assertEquals(String.valueOf(expectedId), response);
	...
	void getBatteryLevelTest_DroneNotFound() throws Exception {
		when(droneService.getBatteryLevel(DRONE_ID_NOT_FOUND)).thenThrow(new DroneNotFoundException());
		String response = getRequest(HOST + DRONES_URL + "//" + DRONE_ID_NOT_FOUND + GET_BATTERY_LEVEL, 404);
		assertEquals(DRONE_NOT_FOUND_MESSAGE, response);
		verify(droneService, times(1)).getBatteryLevel(any());
	}
	...
	private String getRequest(String url, int status) throws Exception {
		String response = mockMvc.perform(get(url)).andDo(print()).andExpect(status().is(status)).andReturn()
				.getResponse().getContentAsString();
		return response;
	}

	private String postRequest(String url, String contentJSON, int status) throws Exception {
		String response = mockMvc.perform(post(url).contentType(MediaType.APPLICATION_JSON).content(contentJSON))
				.andDo(print()).andExpect(status().is(status)).andReturn().getResponse().getContentAsString();
		return response;
	}

	private String putRequest(String url, String contentJSON, int status) throws Exception {
		String response = mockMvc.perform(put(url).contentType(MediaType.APPLICATION_JSON).content(contentJSON))
				.andDo(print()).andExpect(status().is(status)).andReturn().getResponse().getContentAsString();
		return response;
	}
